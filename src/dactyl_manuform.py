...
thumb_centerrow = thumb_nrows - thumb_centerrow_offset
...
def rows_for_column(column, cluster):
    if cluster == "thumb":
        return thumb_nrows

    if column >= reduced_inner_cols and column < (ncols - reduced_outer_cols):
        return nrows
    else:
        return nrows - 1
...
mount_width = keyswitch_width + 2 * plate_rim_horizontal
mount_height = keyswitch_height + 2 * plate_rim_vertical
...
if default_1U_cluster and thumb_style == "DEFAULT":
    ...
    double_plate_height = (0.95 * sa_double_length - mount_height) / 3
else:
...
def _row_radius(cluster):
    if cluster == "thumb":
        _alpha = thumb_alpha
        _extra_height = thumb_extra_height
    else:
        _alpha = alpha
        _extra_height = extra_height
    return ((mount_height + _extra_height) / 2) / (np.sin(_alpha / 2)) + cap_top_height


def _column_radius(cluster):
    if cluster == "thumb":
        _beta = thumb_beta
        _extra_width = thumb_extra_width
    else:
        _beta = beta
        _extra_width = extra_width

    return (((mount_width + _extra_width) / 2) / (np.sin(_beta / 2))) + cap_top_height


row_radius = _row_radius("main")
thumb_row_radius = _row_radius("thumb")
column_radius = _column_radius("main")
thumb_column_radius = _column_radius("thumb")
column_x_delta = -1 - column_radius * np.sin(beta)
thumb_column_x_delta = -1 - thumb_column_radius * np.sin(thumb_beta)
column_base_angle = beta * (centercol - 2)
thumb_column_base_angle = thumb_beta * (thumb_centercol - 2)
...
def column_offset(column: int, cluster="main") -> list:
    if cluster == "thumb":
        _column_offsets = thumb_column_offsets
    else:
        _column_offsets = column_offsets
    return _column_offsets[column]


def column_alpha_offset(column: int, cluster="main") -> float:
    if cluster == "thumb":
        _column_alpha_offsets = thumb_column_alpha_offsets
    else:
        _column_alpha_offsets = column_alpha_offsets
    return _column_alpha_offsets[column]
...
def single_plate(cylinder_segments=100, side="right"):
    if plate_style in ["NUB", "HS_NUB"]:
        ...
        top_wall = translate(
            top_wall, (0, (tb_border / 2) + (keyswitch_height / 2), plate_thickness / 2)
        )

        lr_border = (mount_width - keyswitch_width) / 2
        left_wall = box(lr_border, mount_height, plate_thickness)
        left_wall = translate(
            left_wall, ((lr_border / 2) + (keyswitch_width / 2), 0, plate_thickness / 2)
        )
    ...
    if plate_style == "FLATPACKED":
        half_width = mount_width / 2.0
        half_height = mount_height / 2.0

        slot = lambda: box(
            flatpacked_slot_width + 0.01,
            flatpacked_slot_height + 0.01,
            mount_thickness * 2 + 0.02,
        )
        ...
        slots = [
            translate(
                slot(),
                (
                    half_width - (flatpacked_slot_width / 2),
                    half_height - (flatpacked_slot_height / 2),
                    mount_thickness - 0.01,
                ),
            ),
            translate(
                slot(),
...
def single_plate_poly(side="right"):
    return project(single_plate(side))
...
def trackball_socket(segments=100, side="right"):
    if trackball_modular:
        hole_diameter = ball_diameter + 2 * (
            ball_gap + ball_wall_thickness + trackball_modular_clearance
        )
        ...
        cutter = cylinder(hole_diameter / 2, ring_height + 0.2)
...
def column_angle(column, cluster="main"):
    if cluster == "thumb":
        _beta = thumb_beta
        _centercol = thumb_centercol
    else:
        _beta = beta
        _centercol = centercol
    return _beta * (_centercol - column)
...
def apply_key_geometry(
    ...
    c_angle = column_angle(column, cluster=cluster)
    if cluster == "thumb":
        _column_radius = thumb_column_radius
        _row_radius = thumb_row_radius
        _alpha = thumb_alpha
        ...
        _column_x_delta = column_x_delta
        _tenting_angle = tenting_angle
        _keyboard_z_offset = keyboard_z_offset

    if column_style == "orthographic":
        column_z_delta = _column_radius * (1 - np.cos(c_angle))
        shape = translate_fn(shape, [0, 0, -_row_radius])
        shape = rotate_x_fn(
            shape, _alpha * (_centerrow - row) + column_alpha_offset(column, cluster)
        )
        shape = translate_fn(shape, [0, 0, _row_radius])
        shape = rotate_y_fn(shape, c_angle)
        shape = translate_fn(
            shape, [-(column - _centercol) * _column_x_delta, 0, _column_z_delta]
        )
        ...
        shape = translate_fn(shape, [0, 0, -(_row_radius + fixed_z[column])])
        shape = rotate_x_fn(
            shape,
            _alpha * (_centerrow - row) + column_alpha_offset(column, cluster=cluster),
        )
        shape = translate_fn(shape, [0, 0, _row_radius + fixed_z[column]])
        shape = rotate_y_fn(shape, fixed_tenting)
        shape = translate_fn(shape, [0, column_offset(column, cluster=cluster)[1], 0])
        ...
        shape = translate_fn(shape, [0, 0, _row_radius])
        shape = translate_fn(shape, [0, 0, -_column_radius])
        shape = rotate_y_fn(shape, c_angle)
        shape = translate_fn(shape, [0, 0, _column_radius])
        shape = translate_fn(shape, column_offset(column, cluster=cluster))

    shape = rotate_y_fn(shape, _tenting_angle)
    shape = translate_fn(shape, [0, 0, _keyboard_z_offset])
...
def key_place(shape, column, row, cluster="main"):
    debugprint("key_place()")
    return apply_key_geometry(
        shape, translate, x_rot, y_rot, z_rot, column, row, cluster=cluster
    )
...
def key_position(position, column, row, cluster="main"):
...
def key_holes(side="right", cluster="main"):
    ...
    holes = []
    hole_polys = []
    for column in range(_ncols):
        for row in range(_nrows):
            if (
                cluster == "thumb"
                or (reduced_inner_cols <= column < (ncols - reduced_outer_cols))
                or (not row == lastrow)
            ):
                holes.append(
                    key_place(single_plate(side=side), column, row, cluster=cluster)
                )
                if flatpacked:
                    hole_polys.append(single_plate_poly(side=side))

    shape = union(holes)

    if flatpacked:
        return shape, arrange_polys(hole_polys)
...
def plate_pcb_cutouts(side="right"):
    ...
    for column in range(ncols):
        for row in range(nrows):
            if (reduced_inner_cols <= column < (ncols - reduced_outer_cols)) or (
                not row == lastrow
            ):
...
def caps(cap_type="MX", cluster="main"):
    caps = None
    if cluster == "thumb":
        _ncols = thumb_ncols
        _nrows = thumb_nrows
    ...
    for column in range(_ncols):
        for row in range(_nrows):
            if (
                ...
                if caps is None:
                    caps = key_place(keycap(), column, row, cluster=cluster)
                else:
                    caps = add(
                        [caps, key_place(keycap(), column, row, cluster=cluster)]
                    )
...
## Supports ##
##############
"""Supports for the flatpacked option

key supports are the supports directly under the key plates, which handle the front-to-back rotation
base supports are the supports on the base, which handle the left-to-right rotation
"""


def key_supports(cluster):
    print("key_supports()")
    _ncols = thumb_ncols if cluster == "thumb" else ncols
    supports = [
        key_support(i, side, cluster)
        for i in range(_ncols)
        for side in ["left", "right"]
    ]
    return union(support for support, _ in supports), arrange_polys(
        [poly for _, poly in supports]
    )


def key_support_offset(column_side, face):
    if column_side == "right" and face == "outside":
        return mount_width / 2
    if column_side == "right" and face == "middle":
        return (mount_width / 2) - (flatpacked_thickness / 2)
    if column_side == "right" and face == "inside":
        return (mount_width / 2) - flatpacked_thickness
    if column_side == "left" and face == "outside":
        return -mount_width / 2
    if column_side == "left" and face == "middle":
        return (-mount_width / 2) + (flatpacked_thickness / 2)
    if column_side == "left" and face == "inside":
        return (-mount_width / 2) + flatpacked_thickness

    raise Exception(f"no offset for {column_side},{face}")


def key_support_bottom_point(
    column_side, column, row, vside, face="outside", offsets=(0, 0, 0), cluster="main"
):
    ...
    offset = key_support_offset(column_side, face)
    direction = 1 if vside == "top" else -1
    ...
    return key_position(
        (
            offset + x_offset,
            mount_height / 2 * direction + y_offset,
            -_flatpacked_key_support_height + z_offset,
        ),
        column,
        row,
        cluster=cluster,
...
def key_support(column, column_side, cluster):
    ...
    start_points = [
        key_position(
            (
                offset,
                (mount_height / 2)
                + flatpacked_key_support_end_margin
                + flatpacked_cut_margin,
                z_offset,
            ),
            column,
            0,
        ...
        for z_offset in [
            plate_thickness + flatpacked_cut_margin,
            -flatpacked_key_support_height - flatpacked_cut_margin,
        ]
    ]
    ...
    first_bottom_key_start, first_bottom_key_end = [
        key_support_bottom_point(
            ...
            "top",
            cluster=cluster,
            offsets=(0, 0, -flatpacked_cut_margin),
        ),
        key_support_bottom_point(
            column_side,
            column,
    ...
    top_points = [
        ...
        for point in [
            key_position(
                (offset, mount_height / 2, plate_thickness + flatpacked_cut_margin),
                column,
                row,
                cluster,
            ),
            key_position(
                (
                    offset,
                    (mount_height / 2) - flatpacked_slot_height - flatpacked_cut_margin,
                    plate_thickness + flatpacked_cut_margin,
                ),
                column,
                row,
            ...
            key_position(
                (offset, (-mount_height / 2) + flatpacked_slot_height + flatpacked_cut_margin, flatpacked_cut_margin),
                column,
                row,
                cluster,
            ),
            key_position(
                (offset, (-mount_height / 2) + flatpacked_slot_height + flatpacked_cut_margin, plate_thickness + flatpacked_cut_margin),
                column,
                row,
                cluster,
            ),
            key_position(
                (offset, -mount_height / 2, plate_thickness + flatpacked_cut_margin), column, row, cluster
            ),
        ]
    ]
    ...
    # rotate points to share z plane
    planar_points = [
        rotate_around_y(point, -c_angle - _tenting_angle - deg2rad(90))
        for point in points
    ]

    poly = polyline([(x, y) for (x, y, z) in planar_points])

    extrusion_offset = (
        flatpacked_thickness / 2 if column_side == "left" else -flatpacked_thickness / 2
    )
    extruded = translate(
        extrude_poly(poly, height=flatpacked_thickness), (0, 0, extrusion_offset)
    )

    _, _, z = planar_points[0]  # use any z plane point to translate z lost by polyline

    return (
        union(
            [
                y_rot(
                    translate(extruded, (0, 0, z)),
                    c_angle + _tenting_angle + deg2rad(90),
                )
            ]
        ),
        poly,
...
def key_support_base_support_cuts(column, column_side, cluster):
    def _cut(vside):
        ...
        (
            ...
            "outside",
            vside,
            (0, 0, -flatpacked_key_support_joint_height - flatpacked_cut_margin),
            cluster=cluster,
        )
        ...
        return [
            (bottom_left_x, bottom_left_y - flatpacked_cut_margin, bottom_left_z),
            (top_left_x, top_left_y - flatpacked_cut_margin, z),
            (top_right_x, top_right_y + flatpacked_cut_margin, z),
            (bottom_right_x, bottom_right_y + flatpacked_cut_margin, bottom_right_z),
        ]

    return [_cut("top"), _cut("bottom")]


def key_support_combine_bottom_points(key_support_points, cuts):
    bottom_points = []
    cuts_iter = iter(cuts)
    cut = next(cuts_iter)
    for bottom_key_point in key_support_points:
        _, key_point_y, _ = bottom_key_point
        _, cut_left_y, _ = cut[0] if cut is not None else (0, 0, 0)
        _, cut_right_y, _ = cut[3] if cut is not None else (0, 0, 0)

        if cut is not None and cut_left_y > key_point_y:
            bottom_points.extend(cut)
            cut = next(cuts_iter, None)

        # skip bottom point if it is inside cut
        if not (cut_left_y > key_point_y > cut_right_y):
            bottom_points.append(bottom_key_point)
    return bottom_points


def base_supports(cluster):
    print("base_supports()")
    top, top_corners, top_poly = base_support("top", cluster)
    bottom, bottom_corners, bottom_poly = base_support("bottom", cluster)
    return (
        union([top, bottom]),
        [*top_corners, *bottom_corners],
        arrange_polys([top_poly, bottom_poly]),
    )


def base_support_key_support_intersection(
    y, col, column_side, face, vside, offsets, cluster
):
    def _column_bound_coords(
        y, col, column_side, face
    ) -> tuple[tuple[float, float, float], tuple[float, float, float]] | None:
        rows = (
            range(rows_for_column(col, cluster))
            if vside == "top"
            else reversed(range(rows_for_column(col, cluster)))
        )
        bounds: list[Literal["top", "bottom"]] = (
            ["top", "bottom"] if vside == "top" else ["bottom", "top"]
        )

        checks = [(row, bound) for row in rows for bound in bounds]

        betweens = lambda: pairwise(checks)

        for between in betweens():
            (row_a, bound_a), (row_b, bound_b) = between
            a_x, a_y, a_z = key_support_bottom_point(
                column_side, col, row_a, bound_a, face, offsets, cluster
            )
            b_x, b_y, b_z = key_support_bottom_point(
                column_side, col, row_b, bound_b, face, offsets, cluster
            )

            if vside == "top" and b_y <= y <= a_y:
                return ((a_x, a_y, a_z), (b_x, b_y, b_z))
            if vside == "bottom" and a_y <= y <= b_y:
                return ((a_x, a_y, a_z), (b_x, b_y, b_z))
        raise Exception(f"no bounds found for {col}, {y} in {list(betweens())}")

    (a_x, a_y, a_z), (b_x, b_y, b_z) = _column_bound_coords(y, col, column_side, face)
    if a_y > b_y:
        (a_x, a_y, a_z), (b_x, b_y, b_z) = (b_x, b_y, b_z), (a_x, a_y, a_z)

    ratio = (y - a_y) / (b_y - a_y)

    return (a_x + (b_x - a_x) * ratio), y, (a_z + (b_z - a_z) * ratio)


def base_support_y(vside, face="middle", cluster="main"):
    def _y_extension(column):
        row = 0 if vside == "top" else rows_for_column(column, cluster)
        x, y, z = key_support_bottom_point("left", column, row, vside, cluster=cluster)
        return abs(y)

    _ncols = thumb_ncols if cluster == "thumb" else ncols

    # needs to consider least extended column for this side, otherwise we'll have a key support
    # which doesn't actually extend to the base support
    least_extended_column = min(range(_ncols), key=_y_extension)

    row = 0 if vside == "top" else rows_for_column(least_extended_column, cluster) - 1
    x, y, z = key_support_bottom_point(
        "left", least_extended_column, row, vside, cluster=cluster
    )

    # offset y to account for key support not extending so far across entire y axis
    offset_y = (
        y - flatpacked_base_support_offset
        if vside == "top"
        else y + flatpacked_base_support_offset
    )
...
def base_support(vside="top", cluster="main"):
    ...
    left_corner_x, _, left_corner_z = base_support_key_support_intersection(
        ...
        "outside",
        vside,
        (-flatpacked_thickness, 0, -flatpacked_thickness),
        cluster,
    )
    left_edge_x, _, left_edge_z = base_support_key_support_intersection(
        y,
        0,
        "left",
        "outside",
        vside,
        (-flatpacked_thickness, 0, 2 * flatpacked_thickness),
        cluster,
    )
    right_edge_x, _, right_edge_z = base_support_key_support_intersection(
        y,
        _ncols - 1,
        "right",
        "outside",
    ...
    right_corner_x, _, right_corner_z = base_support_key_support_intersection(
        y,
        _ncols - 1,
        "right",
        "outside",
    ...
    return (
        translate(
            x_rot(extrude_poly(poly, height=flatpacked_thickness), deg2rad(90)),
            (0, y, 0),
        ),
...
def finger_joint(x_start, x_end, z, inverted=False, position="upright"):
    ...
    def _otherside(index):
        if index % 2 == 1:
            return -1
        else:
            return 1

    # using half cut margin, because in this case margin counts double, as both parts of the finger joint have it
    corners = [
        corner
        for index, down in zip(range(number_of_joints), cycle([inverted, not inverted]))
        for corner in [
            (x_start + (box_size * index) + (sign * flatpacked_cut_margin / 2 * _otherside(index)), z + (down * (flatpacked_thickness + (-1 * flatpacked_cut_margin)))),
            (x_start + (box_size * (index + 1)) + (sign * flatpacked_cut_margin / 2 * _otherside(index + 1)), z + (down * (flatpacked_thickness + (-1 * flatpacked_cut_margin)))),
        ]
    ]
...
def flatpacked_baseplate(side, main_corners, thumb_corners):
    ...
    main_top_finger_joints = finger_joint(
        main_top_left[0],
        main_top_right[0],
        main_top_left[1] - (flatpacked_thickness / 2),
        inverted=False,
    )
    main_bottom_finger_joints = finger_joint(
        main_bottom_right[0],
        main_bottom_left[0],
        main_bottom_left[1] - (flatpacked_thickness / 2),
        inverted=True,
    )
    thumb_bottom_finger_joints = _thumb_transform(
        finger_joint(
            thumb_bottom_right[0],
            thumb_bottom_left[0],
            thumb_bottom_left[1] - (flatpacked_thickness / 2),
            inverted=True,
        )
    )
    thumb_top_finger_joints = _thumb_transform(
        finger_joint(
            thumb_top_left[0],
            thumb_top_right[0],
            thumb_top_left[1] - (flatpacked_thickness / 2),
        )
    )
    ...
    return (
        translate(
            extrude_poly(poly, height=flatpacked_thickness),
            (0, 0, flatpacked_thickness / 2),
        ),
        arrange_polys([poly]),
    )
...
def arrange_polys(polys):
    result = [
        translate(poly, [index * 200, poly_y[0], 0]) for index, poly in enumerate(polys)
    ]
...
def thumborigin():
    ...
    if thumb_style == "MINIDOX":
        origin[1] = origin[1] - 0.4 * (minidox_Usize - 1) * sa_length
        ...
        if default_1U_cluster:
            shape_list.append(
                default_thumb_tr_place(
                    rotate(rotate(shape, (0, 0, 90)), [0, 0, thumb_plate_tr_rotation])
                )
            )
            shape_list.append(
                default_thumb_tr_place(
            ...
            shape_list.append(
                default_thumb_tl_place(rotate(shape, [0, 0, thumb_plate_tl_rotation]))
            )
...
def default_thumb_15x_layout(shape, cap=False, plate=True):
    ...
    if plate:
        if cap:
            ...
            cap_list.append(
                default_thumb_tr_place(rotate(shape, [0, 0, thumb_plate_tr_rotation]))
            )
            return add(cap_list)
        else:
            shape_list = [
...
def adjustable_square_plate(Uwidth=1.5, Uheight=1.5):
    ...
    shape = difference(
        shape, [box(mount_width - 0.01, mount_height - 0.01, 2 * web_thickness)]
    )
...
def thumbcaps(side="right", style_override=None):
    ...
    elif _thumb_style == "DACTYLIC":
        return dactylic_thumbcaps()
...
def default_thumb(side="right"):
    ...
    shape = union(
        [shape, default_thumb_15x_layout(rotate(single_plate(side=side), (0, 0, -90)))]
    )
...
def tbjs_thumb_position_rotation():
    ...
    shift = [
        -0.9 * tbjs_key_diameter / 2 + 27 - 42,
        -0.1 * tbjs_key_diameter / 2 + 3 - 25,
        -5,
    ]
...
def tbcj_holder():
    ...
    shape = difference(shape, [cylinder(tbcj_inner_diameter / 2, tbcj_thickness + 0.1)])
...
############################
# Dactylic THUMB CLUSTER
############################


def dactylic_thumb(side):
    pass


def dactylic_thumbcaps():
    return caps(cluster="thumb")
            ...
            wall_brace(
                ...
                (
                    lambda sh: left_key_place(
                        sh, cornerrow, -1, side=ball_side, low_corner=True
                    )
                ),
                -1,
                0,
...
def minidox_thumb_walls(skeleton=False):
    ...
    shape = union(
        [
            wall_brace(
                minidox_thumb_tr_place,
                0,
                -1,
                minidox_thumb_post_br(),
                minidox_thumb_tr_place,
                0,
                -1,
                minidox_thumb_post_bl(),
            )
        ]
    )
    shape = union(
...
def external_mount_hole():
    ...
    undercut = box(external_holder_width + 8, 10.0, external_holder_height + 8 + 0.1)
...
def pcb_screw_hole():
    ...
    hole = translate(hole, (0, 0, pcb_screw_hole_size[1] / 2 - 0.1))
...
def generate_trackball(pos, rot):
    ...
    sensor = translate(sensor, (0, 0, 0.001))
...
def thumb_screw_insert(bottom_radius, top_radius, height, offset=None, side="right"):
    ...
    if _thumb_style == "MINI":
        ...
        xypositions[0][1] = xypositions[0][1] - 0.4 * (minidox_Usize - 1) * sa_length
...
def screw_insert_all_shapes(bottom_radius, top_radius, height, offset=0, side="right"):
    ...
    shape = (
        ...
        translate(
            screw_insert(0, cornerrow, bottom_radius, top_radius, height, side=side),
        ...
        translate(
            screw_insert(
                lastcol, cornerrow, bottom_radius, top_radius, height, side=side
            ),
            (0, 0, offset),
        ),
...
def thumb_screw_insert_outers(offset=0.0, side="right"):
    ...
    return thumb_screw_insert(
        bottom_radius, top_radius, height, offset=offset, side=side
    )
...
def wire_posts():
    ...
    shape = union(
        [shape, default_thumb_ml_place(wire_post(-1, 6).translate([0, 0, -2.5]))]
    )
    shape = union(
        [shape, default_thumb_ml_place(wire_post(1, 0).translate([5, 0, -2]))]
    )
...
def model_side(side="right"):
    ...
    if flatpacked:
        main_key_holes, main_key_holes_poly = key_holes(side=side)
        main_base_support, main_base_corners, main_base_supports_poly = base_supports(
            cluster="main"
        )
        main_key_supports, main_key_supports_poly = key_supports(cluster="main")

        shape = union(
            [
                main_key_holes,
                color([1, 0, 0], 0.5)(main_key_supports),
                color([0, 1, 0], 0.5)(main_base_support),
            ]
        )

        thumb_key_holes, thumb_key_holes_poly = key_holes(side=side, cluster="thumb")
        thumb_key_supports, thumb_key_supports_poly = key_supports(cluster="thumb")
        (
            thumb_base_support,
            thumb_base_corners,
            thumb_base_supports_poly,
        ) = base_supports(cluster="thumb")

        thumb_section = union(
            [
                thumb_key_holes,
                color([1, 0, 0], 0.5)(thumb_key_supports),
        ...
        base, base_poly = baseplate(
            side="right",
            main_corners=main_base_corners,
            thumb_corners=thumb_base_corners,
        )
        base = color([0, 0, 1], 0.5)(base)
        ...
        export_file(
            shape=union(
                [
                    main_base_supports_poly,
                    main_key_supports_poly,
                    main_key_holes_poly,
                    thumb_base_supports_poly,
        ...
        if debug_exports:
            export_file(
                shape=shape, fname=path.join(r"..", "things", r"debug_key_plates")
            )
        ...
        if (
            trackball_in_wall
            and (side == ball_side or ball_side == "both")
            and separable_thumb
        ):
        ...
        if debug_exports:
            ...
            export_file(
                shape=thumb_test,
                fname=path.join(
                    r"..", "things", r"debug_thumb_test_{}_shape".format(side)
                ),
            )
        ...
        if ("TRACKBALL" in thumb_style) and (side == ball_side or ball_side == "both"):
            ...
            if debug_exports:
                export_file(
                    shape=thumb_section,
                    fname=path.join(
                        r"..", "things", r"debug_thumb_test_1_shape".format(side)
                    ),
                )
            ...
            if debug_exports:
                export_file(
                    shape=thumb_section,
                    fname=path.join(
                        r"..", "things", r"debug_thumb_test_2_shape".format(side)
                    ),
                )
            ...
            if debug_exports:
                export_file(
                    shape=thumb_section,
                    fname=path.join(
                        r"..", "things", r"debug_thumb_test_3_shape".format(side)
                    ),
                )
            ...
            if debug_exports:
                export_file(
                    shape=thumb_section,
                    fname=path.join(
                        r"..", "things", r"debug_thumb_test_4_shape".format(side)
                    ),
                )

        if plate_pcb_clear:
            thumb_section = difference(
                thumb_section, [thumb_pcb_plate_cutouts(side=side)]
            )
        ...
        if debug_exports:
            export_file(
                shape=thumb_section,
                fname=path.join(
                    r"..", "things", r"debug_thumb_test_5_shape".format(side)
                ),
            )
        ...
        if separable_thumb:
            ...
            if debug_exports:
                export_file(
                    shape=main_shape,
                    fname=path.join(
                        r"..", "things", r"debug_thumb_test_6_shape".format(side)
                    ),
                )
            if show_caps:
...
def baseplate(wedge_angle=None, side="right", main_corners=None, thumb_corners=None):
    if flatpacked and main_corners and thumb_corners:
        return flatpacked_baseplate(
            side=side, main_corners=main_corners, thumb_corners=thumb_corners
        )
    ...
    if ENGINE == "cadquery":
        ...
        shape = union(
            [case_walls(side=side), *screw_insert_outers(side=side), thumb_section]
        )
...
def run():
    ...
    if symmetry == "asymmetric":
        ...
        export_file(
            shape=tmb_l, fname=path.join(save_path, config_name + r"_thumb_left")
        )
        ...
        export_file(
            shape=lbase, fname=path.join(save_path, config_name + r"_left_plate")
        )
        export_dxf(
            shape=lbase, fname=path.join(save_path, config_name + r"_left_plate")
        )
